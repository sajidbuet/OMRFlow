...
One isolated, behaviour-preserving refactor plus two additive fields - see
...
|---|---|---|---|
| Unit | `tests/unit/test_calibration_service.py` | 27 | Every judgement rule and its documented boundary, against hand-built `ScanResult`s. |
| Integration | `tests/integration/test_calibration_workflow.py` | 10 | The real `RecognitionEngine` against real templates and synthetic sheets: registration-failure-is-never-a-pass, small-vs-large marker offsets, threshold propagation through a real `CalibrationSession`, apply/save/reload, staleness after an edit, pre-Phase-4 document compatibility. |
| GUI | `tests/gui/test_calibration_page.py` | 26 | A-J: launch/navigation, template/scan management, running a scan, overlay geometry exactness, click-to-inspect (including a cross-check that the question-number label formatter agrees with the engine's own numbering), field filtering, threshold controls (asserting **no** worker starts), save/staleness, multi-scan aggregation, the mismatched-template failure path end to end. |
...
```text
pytest tests/unit/test_calibration_service.py -q                → 27 passed
pytest tests/integration/test_calibration_workflow.py -q        → 10 passed
pytest tests/gui/test_calibration_page.py -q -m gui              → 26 passed
pytest tests/gui/test_main_window.py tests/gui/test_scan_page.py
       tests/gui/test_calibration_page.py -q -m gui               → 90 passed
pytest -q -m "not gui"                                            → 1576 passed, 1 skipped
pytest -q -m gui                                                  → 582 passed   (full GUI suite, ~92 min real QThreads)
ruff check .                                                      → All checks passed
mypy                                                               → Success: no issues found in 102 source files
.claude/skills/qtguitesting/scripts/run_gui_smoke_tests.py         → 30/30 checks passed
.claude/skills/qtguitesting/scripts/capture_gui_states.py --only calibration → 5 screenshots written
```
...
Screenshots captured and inspected (`test-output/gui/`, git-ignored):
`calibration_loaded.png`, `calibration_run_clean.png` (full quality summary
and sample table against the real sample), `calibration_overlay_markers.png`
(the top-left marker at 100% zoom - a green square on the detected position,
a blue cross on the expected one, both nearly coincident, labelled "top left
(0.0px)"), `calibration_threshold_ambiguous.png`, and
`calibration_mismatched_failed.png` (a blank preview, "Calibration failed" in
red, "Markers detected: 0 / 4", "Questions: 0 total", the sample table's
Status column reading "FAILED" - inspected directly and confirmed to show no
plausible wrong result of any kind).

A real bug was found and fixed during this work: the qtguitesting harness's
`CalibrationHarness.run_all()` originally connected `run_finished` (a
...
A second, distinct path covers a template that *does* register but no longer
matches the scan's printed geometry: if more than 5% of a sheet's bubbles come
back `usable=False` (their sampling window fell off the rectified page or
