...
| Project template | recorded and reread, relative path survives relocating the project, a deleted template is reported not crashed on, a lone template is adopted and two are never guessed between, discovery order is machine independent, each project keeps its own, a template outside the project or a read-only session is refused, stored paths that escape the project are rejected, a pre-format-3 document still opens |
| Template propagation (GUI) | opening a project loads it on Template, Calibrate and Scan; saving or opening one on the Template screen becomes the project's; a template outside the project is not adopted; switching projects swaps it and clears what the previous project left; a missing template is reported on Calibrate; file dialogs start inside the project |
